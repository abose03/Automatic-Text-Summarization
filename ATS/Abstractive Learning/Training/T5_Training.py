...
num = 1
wandb.init(project="T5-CNN-Summarization_BottleSum_1-340", name=f"Exp-{num}")


# loading train and validation data.
df_train = pd.read_excel('../BottleSum/BottleSum_train_1-340_Summaries.xlsx')
df_val = pd.read_excel('../DATA/combined_test_bottlesum_results.xlsx')
...
train_dataset = T5SummDataset(text=df_train['Summary'].values,
...
val_dataset = T5SummDataset(text=df_val['Summary'].values,
                            summarized_text=df_val['highlights'].values,
                            corrupted_text=df_val['noisy'].values,
                            pronoun_swap = df_val['Pronoun_swap'].values, 
                            deleted_added_noise = df_val['deleted'].values,
                            tokenizer=tokenizer,
                            src_max_length= src_max_length,
                            tgt_max_length= tgt_max_length
                             )
...
for i in range(epochs):
    ...
    if bleu_score > final_bleu_score:
        ...
        model.save_pretrained("T5_Model_BottleSum_1-340")
