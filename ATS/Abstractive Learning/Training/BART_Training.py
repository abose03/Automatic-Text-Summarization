...
from transformers import  BartTokenizer, BartForConditionalGeneration, AdamW, BartConfig, get_cosine_with_hard_restarts_schedule_with_warmup
...
num = "in-article-out-BART"
#wandb.init(project="BART-CNN-Summarization_BART_EXtracts", name=f"Exp-{num}")
wandb.init(project="BART-CNN-Summarization_article-BART", name=f"Exp-{num}")
...
train_dataset = BARTSummDataset(text=df_train['article'].values,
                              summarized_text=df_train['distilbart-cnn-12-6'].values,
                              corrupted_text=df_train['CBART_noisy'].values,
                              pronoun_swap = df_train['CBART_Pronoun_replaces'].values, 
                              deleted_added_noise = df_train['CBART_deleted_added_noise'].values,
                              tokenizer=tokenizer,
...
val_dataset = BARTSummDataset(text=df_val['article'].values,
                            summarized_text=df_val['distilbart-cnn-12-6'].values,
                            corrupted_text=df_val['CBART_noisy'].values,
                            pronoun_swap = df_val['CBART_Pronoun_replaces'].values, 
                            deleted_added_noise = df_val['CBART_deleted_added_noise'].values,
                            tokenizer=tokenizer,
                            src_max_length= src_max_length,
                            tgt_max_length= tgt_max_length
                             )
...
def training_loop():
    ...
    for d in pbar:
        ...
        loss1=loss_fct(logits.view(-1, config.vocab_size), output_ids.view(-1))
        
        # calculating the loss2 with respect to pred and corruption data.
        loss2=loss_fct(logits.view(-1, config.vocab_size), output_ids_corrupted.view(-1))
...
def validation_loop():
    ...
    with torch.no_grad():
        ...
        for d in tqdm(val_dataloader):
            ...
            # generating the summary for the input article.
            preds_ids = model.generate(input_ids, max_length=200, num_return_sequences=1)
...
# doining for epochs num of times.s
for i in range(epochs):
    ...
    if bleu_score > final_bleu_score:
        ...
        model.save_pretrained("BART_Model_article_BART")
